...
class MVPP(object):
    def __init__(self, program, k=1, eps=0.000001, prob_ground=False, binary_rule_belongings={}, max_n=0):
        ...
        self.pc, self.parameters, self.learnable, self.asp, self.pi_prime, self.remain_probs, self.ga_map = self.parse(
            program, prob_ground)
    ...
    def create_selection_mask(self):
        selection_mask = torch.zeros([self.parameters.__len__(), max(par.__len__() for par in self.parameters)],
    ...
    def parse(self, program, prob_ground):
        ...
        for line in lines:
            ...
            if re.match(r'.*[0-1]\.[0-9]*\s.*(;|\.).*', line):
                ...
                out = re.search(
                    r'@[0-9]*\.[0-9]*([a-z][a-zA-Z0-9_]*)\(([0-9]*),([0-9]),([a-z]*[a-zA-Z0-9]*|[a-z]*[a-zA-Z0-9]*,[a-z]*[a-zA-Z0-9]*),([a-z]*[a-zA-Z0-9_]*\))',
                    line.replace(" ", ""), flags=0)
    ...
    def find_all_most_probable_SM_under_query_noWC(self, query):
        ...
        clingo_control.add("base", [], program)
        clingo_control.ground([("base", [])])
        clingo_control.solve([], lambda model: models.append(
            model.symbols(atoms=True)) if model.optimality_proven else None)
        models = [[str(atom) for atom in model] for model in models]
    ...
    def find_k_most_probable_SM_under_query_noWC(self, query='', k=1):
        ...
        return models[-np.minimum(len(models),
                                  k):]  # return the last k models. The models are ordered by optimization values e.g how likely they are
    ...
    def find_SM_vqa_with_same(self, query='', k=1, obj_filter=np.inf, threshold=0.99, vqa_params={}, train=True):
        ...
        for rule_idx, rule in enumerate(self.npp_choices_inftype_sorted):
            ...
            if obj_idx_min < obj_filter and obj_idx_max < obj_filter:
                ...
                if rule_idx in self.binary_rule_belongings:
                    for atom_idx, atom in enumerate(self.pc[rule_idx]):
                        if self.M[rule_idx][atom_idx] > 1 - t:
                            if any("," + ar_filter + ")" in self.pc[rule_idx][atom_idx] for ar_filter in
                                   attr_relation_filter):
                                pi_prime += "0{" + self.pc[rule_idx][atom_idx] + "}1.\n"
        ...
        for split in rel_splits:
            ...
            try:
                with clingo_control.solve([], lambda model: models.append(model.symbols(shown=True)),
                                          async_=True) as hnd:
    ...
    def mvppLearn(self, models, model_idx_list, grad_device):
        ...
        grad_tensor = self.mvppLearnRule(models, model_idx_list, grad_device, probs)
        return grad_tensor

    # Separated from mvppLearn, so that gradient calculations can be tested independently
    ...
    def learn_exact(self, list_of_query, lr=0.01, thres=0.0001, max_iter=None):
        ...
        print("Start learning by exact computation with {} queries...\n\nInitial parameters: {}".format(
            len(list_of_query), self.parameters))
        time_init = time.time()
        check_continue = True
        iteration = 1
    ...
    def learn_by_sampling(self, list_of_query, num_of_samples=50, lr=0.01, thres=0.0001, max_iter=None, num_pretrain=1):
        ...
        print("Start learning by sampling with {} queries...\n\nInitial parameters: {}".format(len(list_of_queries),
                                                                                               self.parameters))
        time_init = time.time()
